...
for first_letter in range(len(airlines_A_Z_href)):
    ...
    for number_of_airlines in range(len(airlines_first_letter_href)) :
        ...
        airline_header = soup_airline.findAll("header", {"class": "td-post-title td-post-title-logo"})
        airline_category = airline_header[0].find("p").text

        # Get airline average rating
        airline_rating_box = soup_airline.findAll("div", {"class": "average_passenger_rating"})
        if len(airline_rating_box) > 0 :
            airline_rating = airline_rating_box[0].find("h4").text[:-4]

        # Put infos in df
        df_airlines = df_airlines.append({ "Airline": airline_name, "Category":  airline_category, "Rating":  airline_rating }, ignore_index=True)

df_airlines.to_csv("airlineratings_categories_and_ratings.csv", sep = ",", index = False)