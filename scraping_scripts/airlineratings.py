...
def collect_reviews_and_infos_on_one_page(comments_airline, reviews_df):
    for feedback in comments_airline :
        # get review
        review = feedback.find_all("div", {"class": "passenger_review_body"})[0].find_all("p")
        if len(review) > 0:
            review = feedback.find_all("div", {"class": "passenger_review_body"})[0].find_all("p")[0].text
        else :
            review = np.nan

        # get rating
        rating = feedback.find_all("div", {"class": "passenger_rating_overall"})[0].find_all("h4")[0].text

        # get country
        country = feedback.find_all("div", {"class": "passenger_review_header"})[0].find_all("p")[0].text
        country = country[country.find('from ')+len('from '):country.rfind(' -')]

        # get date
        date = feedback.find_all("div", {"class": "passenger_review_header"})[0].find_all("p")[0].text
        ...
        for el in li :
            ...
            if len(el.find_all("div", {"class": "rating"})) > 0 :
                rating_value = el.find_all("div", {"class": "rating"})[0].get('style')[7:]
        ...
        recommandation = feedback.find_all("div", {"class": "passenger_ratings"})[0].find_all("li")[-1]
        ...
        # put infos in df
        reviews_df = reviews_df.append({ 'Airline': airline_name, 'Review': review, 'Rating': rating,
                                         'Country' : country, 'Date' : date, 'Class' : cabin_flown,
                                         'Overall Value for Money' : rating_ovm, 'Seat and Cabin Space' : rating_scs,
                                         'Customer Service' : rating_cs, 'In Flight Entertainment' : rating_ife,
                                         'Baggage Handling' : rating_bh, 'Check-in Process' : rating_cip,
                                         'Meals and Beverages' : rating_mb, 'Recommend Airline' : recommend},
                                       ignore_index=True)
    return(reviews_df)
...
def airlineratings_scraping(url):
    ...
    for first_letter in range(len(airlines_A_Z_href)):
        url_first_letter = "https://www.airlineratings.com" + airlines_A_Z_href[first_letter]
        page_first_letter = requests.get(url_first_letter)
        soup_first_letter = BeautifulSoup(page_first_letter.text)
        li_first_letter = soup_first_letter.findAll("div", {"class": "td-page-content"})[0].find_all("li")
        ...
        for number_of_airlines in range(len(airlines_first_letter_href)) :
            ...
            airline_name = soup_airline.findAll("header", {"class": "td-post-title td-post-title-logo"})[0].find_all("h1")[0].text
            comments_airline = soup_airline.findAll("div", {"class": "passenger_review"})

            reviews_df = collect_reviews_and_infos_on_one_page(comments_airline, reviews_df)


            # Reviews from pages 2 to end of this particular airline
            for page_number in range(2, 10000):
                url_airline = airlines_first_letter_href[number_of_airlines] + 'page/' + str(page_number) + '/'
                page_airline = requests.get(url_airline)
                soup_airline = BeautifulSoup(page_airline.text)
                airline_name = soup_airline.findAll("header", {"class": "td-post-title td-post-title-logo"})[0].find_all("h1")[0].text
                comments_airline = soup_airline.findAll("div", {"class": "passenger_review"})
